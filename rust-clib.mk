...
$(filter %.a.d,$(OUTPUTS:%=%.d)): %.a.d: $(SRCFILE) $(EXTERN_LIBS)
	$(RUSTC) $(ALL_RUSTFLAGS) --emit dep-info=$@ --crate-name=$* $<

$(filter %.a,$(OUTPUTS)): %.rlib: %.a.d $(SRCFILE) $(EXTERN_LIBS)
	+$(RUSTC) $(ALL_RUSTFLAGS) --emit dep-info=$@.d --emit link=$@ --crate-type=staticlib --crate-name=$* $<

$(filter %.so.d,$(OUTPUTS:%=%.d)): %.so.d: $(SRCFILE) $(EXTERN_LIBS)
	$(RUSTC) $(ALL_RUSTFLAGS) --emit dep-info=$@ --crate-name=$* $<

$(filter %.so,$(OUTPUTS)): %.so: %.so.d $(SRCFILE) $(EXTERN_LIBS)
	+$(RUSTC) $(ALL_RUSTFLAGS) --emit dep-info=$@.d --emit link=$@ --crate-type=cdylib --crate-name=$* $<
